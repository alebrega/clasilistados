<?PHP
	...
	{ 
		$valor = trim($valor); 
		$valor=db::getInstance()->real_escape_string($valor);
	...
	function limpia_espacios(&$valor) 
	{ 
		$valor = isset($valor) ? $valor : null ;  //me aseguro que la variable exista, a veces, al mismo tiempo, que tenga el valor que yo deseo.
    	$valor = trim($valor); 
	}
    // Fixes MAGIC_QUOTES
